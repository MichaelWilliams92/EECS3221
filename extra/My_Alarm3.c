    ...
    while (1) {
       ...
       printf("%d\n", strcmp(line, snoozeCommand));
        if(strcmp(line, snoozeCommand) == 0) {
         ...
         //else if first argument is in form hh:mm:ss (24 hour clock), change this to seconds, and then send that value to &alarm->seconds, keep everything else the same.  Consider changing so that it can read eithor as AM or PM.  Then, work on making that actual time appear as the original message
